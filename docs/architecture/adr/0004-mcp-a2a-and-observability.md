# ADR 0004: MCP/A2A open standards and an observability harness surface
...
`graphwiki` had no `.ai/` observability surface and no MCP/A2A governance surface
— even though GraphWiki itself ships an MCP server (`src/serve/`, HTTP + stdio)
and already estimates token budgets in its context-loading protocol.
...
- Promote **MCP/A2A** from a mention to a real governance surface:
  `.ai/mcp/registry.json` (MCP-server registry stub — `status: "stub"`, no live
  endpoint resolved, includes a stub entry for GraphWiki's own MCP server) and
...
- Generated `AGENTS.md` carries an explicit **Harness Map** enumerating the six
  context types and documents the static-vs-dynamic context boundary as a
  reviewed, versioned architectural decision.

## Consequences
- Agent drift, cost, and trajectory become auditable; supports the model-routing
  audit defined in ADR-0002.
