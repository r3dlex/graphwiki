...
async function readIfPresent(ctx: RuleContext, file: string): Promise<string | null> {
  try {
  ...
  rules: {
    'gw-graph-001': {
      ...
      async check(ctx) {
        ...
        const source = await readIfPresent(ctx, file);
    ...
    'gw-wiki-001': {
      ...
      async check(ctx) {
        for (const file of await ctx.glob('wiki/**/*.md')) {
    ...
    'gw-wiki-002': {
      ...
      async check(ctx) {
        const writeToRaw = /(?:writeFile|writeFileSync|copyFile|copyFileSync|mkdir|mkdirSync|rename)\s*\([^\n;]*['"`]\/?raw\//;
    ...
    'gw-wiki-003': {
      ...
      async check(ctx) {
        const files = await ctx.glob('wiki/**/*.md');
        const targets = new Set(files.map((file) => file.replace(/^wiki\//, '').replace(/\.md$/, '')));
    ...
    'gw-serve-001': {
      ...
      async check(ctx) {
        ...
        const source = await readIfPresent(ctx, file);
        if (!source) return;
        const closeHandler = source.match(/req\.on\(\s*['"]close['"]\s*,\s*\(\)\s*=>\s*\{([\s\S]*?)\n\s*\}\s*\)/);
        if (!closeHandler || !/clearInterval\(pingInterval\)/.test(closeHandler[1]) || !/sseClients\.delete\(clientHandler\)/.test(closeHandler[1])) {
          violation(ctx, 'SSE close handler must clear pingInterval and remove clientHandler', file);
        }
      },
    },
    'gw-dedup-001': {
    ...
    'gw-dedup-003': {
      ...
      async check(ctx) {
        ...
        const source = await readIfPresent(ctx, file);
        if (source && /effectiveSim|context_boost/.test(source) && !/effectiveSim\s*=\s*Math\.min\(\s*1\s*,/.test(source)) {
