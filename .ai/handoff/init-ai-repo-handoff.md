...
- **done**: Additive v3 governance upgrade applied to `graphwiki` (standalone, depth 0, pnpm). Workflows, traceability, evals scaffold, MCP/A2A, model-routing policy, observability, AI-failure-mode review, command surfaces, phase status files, and handoff generated. Existing `.ai/rules/`, `.ai/skills/`, `.ai/system-prompts/`, `.ai/drift/`, `.memory/`, and both ADR directories preserved.
- **verified**: `pnpm install`, `pnpm test`, `pnpm run build`, `pnpm run typecheck`, and `pnpm run lint` run locally; JSON artifacts validated with `python3 -m json.tool`.
